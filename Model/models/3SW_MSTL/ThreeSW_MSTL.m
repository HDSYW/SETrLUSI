function probPos = ThreeSW_MSTL(sources, target, p1, ~, k)
...
% pyversion D:\pycharm\code\pythonProject\.venv\Scripts\python.exe; % load python
% system('pip install scikit-learn')
KMM = py.importlib.import_module('KMM');
%% Hyper-parameters
...
Xt = temp';
W = py.sklearn.metrics.pairwise.rbf_kernel(pyargs('X',py.numpy.asarray(Xt), 'gamma', 2^p1)); % 
W = W.data.double;
D = diag(sum(W,2));
Lu = D-W;
