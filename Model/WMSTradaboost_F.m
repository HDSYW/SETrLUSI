function [result] = WMSTradaboost_F(Data,ktype,pa)
...
for p1=pa.min:pa.step:pa.max
    ...
    for t = 1:pa.T
        ...
        for k = 1:N
            ...
            for j=1:n
                ws(k).weight(j) = P(j)*yita*exp(-as*abs(TrainPredict{k}(j)-data_ori{k}.Y_train_A(j)));
            end
        ...
        for j=1:m
            wt.weight(j) = wt.weight(j)*yita*exp(alpha(t)*abs(T(j)-Y_train_T(j)));
        end
